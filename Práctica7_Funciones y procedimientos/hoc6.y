...
%%
	...
	stmt: exp	  			{code(pop);}
		...
		| if cond stmt end 		{ /*if que no emplea else*/
						($1)[1] = (Inst)$3; /* parte then */
						($1)[3] = (Inst)$4; } /* terminar si la condicion no se cumple */
		| if cond stmt end ELSE stmt end	{ /*if ocn parte else*/
							($1)[1] = (Inst)$3; /*parte then*/
...
int yylex (){
  	int c;
  	while ((c = getchar ()) == ' ' || c == '\t')  
  		;//Enunciado nulo, salta espacios en blanco
 	if (c == EOF)                            
    	return 0;
  	if (isdigit (c)){
		double d;
...
void defonly(char *s)	/*advertencia en caso de definicion ilegal*/
{
	if(!indef)
...
void run()/* ejecutar hasta al fin de archivo*/
{
	setjmp(begin);
...
int backslash(c) /*obtener el siguiente caracter con las \ interpretadas */
int c;
{
