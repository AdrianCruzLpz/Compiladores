...
Inst *code(Inst f)
{ /* lnstalar una lnstruccion u operando */
	Inst *oprogp = progp;
	if (progp >= &prog[NPROG])
...
{
	sp->u.defn = (Inst)progbase; /* principio de codigo */
	progbase = progp;			 /* el siguiente codigo comienza aqui */
}

void call()
{
	Symbol *sp = (Symbol *)pc[0]; /*entrada en la tabla de simbolos*/
...
void funcret()
{
	...
	d = pop(); /* preservar el valor de regreso a la funcion*/
	ret();
	push(d);
}
