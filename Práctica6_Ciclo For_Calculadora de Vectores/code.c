...
//Funcion forcode para la practica 6
void forcode(){
    Datum d;
    Inst* savepc = pc;
    execute(savepc + 4);
    execute(*((Inst **)(savepc)));
    //Sacamos la instrucción
    d = pop();
    while(d.val){
        execute(* ( (Inst **)(savepc + 2)));   //Cuerpo del for
        execute(* ( (Inst **)(savepc + 1)));   //Ultimo campo del for
        pop();
        execute(*((Inst **)(savepc))); //Condicion
        d = pop();
    }
    pc = *((Inst **)(savepc + 3)); //Siguiente posicion
}
